...
app.get('/food/list', async (req, res) => {
  const { foodNo} = req.query;
  try {
    const result = await connection.execute(`SELECT * FROM FOOD_TYPES`);
    const columnNames = result.metaData.map(column => column.name);
    // 쿼리 결과를 JSON 형태로 변환
    const rows = result.rows.map(row => {
      // 각 행의 데이터를 컬럼명에 맞게 매핑하여 JSON 객체로 변환
...
app.get('/food/insert', async (req, res) => {
  ...
  try {
    await connection.execute(
      `INSERT INTO FOOD_TYPES (FOOD_TYPE_NAME, DESCRIPTION, RECOMMEND_MOOD, RECOMMEND_DESCRIPTION) VALUES (:name, :descr, :mood, :reco)`,
      [name, descr, mood, reco],
      { autoCommit: true }
